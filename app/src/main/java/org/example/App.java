...
public class App {
    public static void main(String[] args) {
        ...
        GeneticAlgorithm ga = new GeneticAlgorithm(
                new UniformCrossover<Integer>(0.5),
                0.9,
                new BinaryMutation(),
                0.01,
	...
	class IntegerChromosome extends BinaryChromosome {
        ...
        for (int i = 0; i < population; i++) {
            chromosomes.add( new IntegerChromosome( 
            
		    Stream.concat(
		        Collections.nCopies(length/5, 0).stream(),
        ...
        Chromosome fittest = ga.evolve(new ElitisticListPopulation(chromosomes, chromosomes.size()*2, 0.05), new FixedGenerationCount(generations)).getFittestChromosome();;
